...
from core.logic import find_user_by_email, get_language, \
    ...
    create_schedule_entry, get_timetable, oauth2_scheme, cancel_lesson, set_users, teachers_list
...
def to_book(date, teacher: EmailStr, db: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    try:
        ...
        if len(check_timetable(teacher, 'teacher', db, date)) > 0 or len(
                check_timetable(student, 'student', db, date)) > 0:
...
def find_free_teachers(from_date, to_date=None, language=None, db: Session = Depends(get_db),
    ...
    try:
        ...
        list_teachers = teachers_list(db, Teacher, language)

...
from core.Commons import add_into_db, delete_in_db, delete_by_id, find_data_by_id, \
    find_by_name, check_to_date_param, find_user_by_token
...
def check_timetable(user, role, db_connect: Session, from_date, to_date=None):
    try:
        ...
        return db_connect.query(MODEL). \
            filter(dict_student_teacher.get(role) == user,
                   MODEL.day >= from_date, MODEL.day <= to_date). \
            order_by(MODEL.day.asc()).all()
    except Exception:
...
def to_book(date, teacher_name, db_connect: Session = Depends(get_db),
    ...
    try:
        ...
        if len(check_timetable(teacher_id, 'teacher', db_connect, date)) > 0 or len(
                check_timetable(student_id, 'student', db_connect, date)) > 0:
...
def list_of_classes_for_teacher(from_date, to_date=None, db_connect: Session = Depends(get_db),
                                token: str = Depends(oauth2_scheme)):
    try:
...
def list_of_classes_for_student(from_date, to_date=None, db_connect: Session = Depends(get_db),
                                token: str = Depends(oauth2_scheme)):
    try:
        checking_for_access_rights(token, 'student')
...
def cancel_lesson(date, role, db_connect: Session, token):
    try:
        ...
        timetable = check_timetable(dict_user_teacher.get(role), role, db_connect, from_date=date)
        if len(timetable) < 1:
...
def cancel_lesson_by_teacher(date, db_connect: Session = Depends(get_db),
                             token: str = Depends(oauth2_scheme)):
    return cancel_lesson(date, 'teacher', db_connect, token)


def cancel_lesson_by_student(date, db_connect: Session = Depends(get_db),
                             token: str = Depends(oauth2_scheme)):
    try:
...
def find_free_teachers(from_date, to_date=None, language=None,
    ...
    try:
        ...
        for teacher in list_teachers:
            lessons_in_dates = check_timetable(teacher.id, 'teacher',
                                               db_connect, from_date, to_date)
...
def timetable_list(db_connect, from_date=None, to_date=None):
    if from_date is not None:
        to_date = check_to_date_param(from_date, to_date)
        timetable = db_connect.query(MODEL). \
...
def get_book_list(db_connect: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    try:
...
def teacher_statistic_for_admin(from_date=None, to_date=None, db_connect: Session = Depends(get_db),
                                token: str = Depends(oauth2_scheme)):
    try:
        checking_for_access_rights(token, 'admin')

...
from core.Commons import add_into_db, check_by_name, read_from_csv, \
    create_full_name
...
def get_students_for_admin(db_connect: Session = Depends(get_db), token: str = Depends(oauth2_scheme)):
    try:

...
from core.Timetable import (
    ...
    find_free_teachers,
    teacher_statistic_for_admin,
    list_of_classes_for_teacher,
    get_book_list,
    cancel_lesson_by_teacher,
...
async def login(
    ...
    try:
        user_and_role_by_email = find_user_by_email(
            detail.username, db_connect, return_role=True
        )
...
@app.post(
    "/add_teacher_from_csv", status_code=status.HTTP_201_CREATED, tags=["add from csv"]
)
...
@app.post(
    "/add_student_from_csv", status_code=status.HTTP_201_CREATED, tags=["add from csv"]
)
...
async def list_students_classes(
    user: CreateUserReuest = Depends(list_of_classes_for_student),
):
...
async def cancel_lesson_for_student(
    user: CreateUserReuest = Depends(cancel_lesson_by_student),
):
...
async def list_of_available_teachers(
...
async def list_teachers_classes(
    classes: CreateUserReuest = Depends(list_of_classes_for_teacher),
):
...
@app.post(
    "/teacher/add_language", status_code=status.HTTP_201_CREATED, tags=["teacher"]
)
...
@app.delete(
    "/teacher/remove_language", status_code=status.HTTP_200_OK, tags=["teacher"]
)
async def remove_languages(
    result: CreateUserReuest = Depends(remove_language_by_teacher),
):
...
async def cancel_lesson_for_teacher(
    result: CreateUserReuest = Depends(cancel_lesson_by_teacher),
):
